...
import { settings, heightfield, world } from '../../settings.js'
...
async function createFireflies(datGUI) {
  ...
  for(let i = 0; i < firefliesCount; i++){
    ...
    positionArray[i * 3 + 0] = xPos + heightfield.offset.x
    positionArray[i * 3 + 1] = yPos + 10 + (Math.random() * 15 -10)
    positionArray[i * 3 + 2] = zPos + heightfield.offset.z

    scaleArray[i] = 2 + Math.random() * 100
  }
  ...
  world.elements.fireflies  = fireflies       // Reference


  // Debug
  settings.elements.datGUIFolders.particlesFolder.add(fireflies, 'visible').name('Show fireflies!')
  settings.elements.datGUIFolders.particlesFolder.add(firefliesMaterial.uniforms.uSize, 'value')
    .min(0).max(500).step(1).name('Firefly size')
    .onChange( function(){      
      const intensity = this.object.value / 500 * 5
      settings.lights.ambientLight.intensity = intensity
      world.elements.lights.ambientLight.intensity = intensity
